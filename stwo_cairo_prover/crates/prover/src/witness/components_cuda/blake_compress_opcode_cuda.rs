...
fn write_trace_cuda(
    ...
    // let blake_round_input_vec = blake_round_state.inputs.iter().map(|x| x.device_ptr).collect_vec();
    ...
    unsafe {
        // bindings::generate_blake_compress_opcode_traces(
        //     traces_vec.as_ptr(),
        //     // 所有 lookup ptrs
...
impl CudaInteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        unsafe {
            ...
            // bindings::generate_blake_compress_opcode_interaction_traces(
            //     blake_round_ptr,
...
pub mod tests {
    ...
    fn test_blake_compress_opcode_cpu_ref() {
        ...
        println!("input_state.casm_states_by_opcode.blake_compress_opcode.len(): {:?}", input_state.casm_states_by_opcode.blake_compress_opcode.len());
        ...
        println!("blake_compress_claim log_size: {:?}", blake_compress_claim.log_size);
        ...
        println!("blake_compress_interaction_claim.claimed_sum: {:?}", blake_compress_interaction_claim.claimed_sum);
        // println!("trace: {:?}", trace);
