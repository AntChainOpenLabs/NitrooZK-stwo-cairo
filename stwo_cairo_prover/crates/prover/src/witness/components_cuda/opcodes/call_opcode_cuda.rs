#![allow(unused_parens)]
use cairo_air::components::call_opcode::{Claim, InteractionClaim};
...
fn write_trace_cuda(
    ...
    unsafe {
        bindings_airs::generate_call_opcode_traces(
            traces_vec.as_ptr(),
...
impl CudaInteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        unsafe {
            ...
            bindings_airs::generate_call_opcode_interaction_traces(
...
pub mod tests {
    ...
    use crate::witness::components_cuda::call_opcode_cuda;
    ...
    use cairo_air::components::call_opcode::Eval;
    ...
    #[test]
    fn test_call_opcode_cpu_ref() {
        // Create a simple call opcode test case (matches test_call from opcodes.rs)
        ...
        // Check that we have call_opcode states
        if input_state.casm_states_by_opcode.call_opcode.is_empty() {
            println!("Warning: No call_opcode states generated. Test will be skipped.");
            return;
        }

        let call_gen = call_opcode::ClaimGenerator::new(input_state.casm_states_by_opcode.call_opcode);
        ...
        println!("call_opcode_claim log_size: {:?}", call_claim.log_size);
        ...
        println!("call_opcode_interaction_claim.claimed_sum: {:?}", call_interaction_claim.claimed_sum);
    ...
    #[test]
    fn test_call_opcode_trace_gen_by_cpu_and_verify_by_cuda() {
        // Create a simple call opcode test case (matches test_call from opcodes.rs)
        let instructions = casm! {
            call rel 2;
            call abs [fp - 1];
        ...
        let call_gen = call_opcode::ClaimGenerator::new(
                input_state.casm_states_by_opcode.call_opcode,
            );
        ...
        println!("call_opcode_claims log_size: {:?}", call_claim.log_size);
    ...
    #[test]
    fn test_call_opcode_trace_gen_by_cuda_and_verify_by_cpu() {
        let instructions = casm! {
            call rel 2;
            call abs [fp - 1];
            [ap] = 1, ap++;
        ...
        let call_gen = call_opcode_cuda::CudaClaimGenerator::new(input_state.casm_states_by_opcode.call_opcode);
    ...
    #[test]
    fn test_call_opcode_trace_gen_by_cuda_and_verify_by_cuda() {
        let instructions = casm! {
            call rel 2;
            call abs [fp - 1];
            [ap] = 1, ap++;
