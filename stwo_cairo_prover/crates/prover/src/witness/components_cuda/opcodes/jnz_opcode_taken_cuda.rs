#![allow(unused_parens)]
use cairo_air::components::jnz_opcode_taken::{Claim, InteractionClaim};
...
fn write_trace_cuda(
    ...
    let memory_id_to_big_transposed_big_values_vec = memory_id_to_big_state.transposed_big_values
        ...
        .collect_vec();
    unsafe {
        bindings_airs::generate_jnz_opcode_taken_traces(
            traces_vec.as_ptr(),
...
impl CudaInteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        unsafe {
            ...
            bindings_airs::generate_jnz_opcode_taken_interaction_traces(
...
pub mod tests {
    ...
    use crate::witness::components::{memory_address_to_id, memory_id_to_big, verify_instruction};
    use crate::witness::components_cuda::jnz_opcode_taken_cuda;
    ...
    use cairo_air::components::jnz_opcode_taken::Eval;
    use stwo::core::fields::m31::M31;
    use stwo_cairo_common::preprocessed_columns::preprocessed_trace::testing_preprocessed_tree;
    use crate::witness::components::jnz_opcode_taken;
    use cairo_lang_casm::casm;
    use crate::test_utils::input_from_plain_casm;
    use cairo_air::components::jnz_opcode_taken::Component;
    use itertools::Itertools;
    use stwo::stwo_cuda::base_field_vec::BaseFieldVec;
    ...
    #[test]
    fn test_jnz_opcode_taken_cpu_ref() {
        ...
        // Check that we have jnz_opcode_taken states
        if input_state.casm_states_by_opcode.jnz_opcode_taken.is_empty() {
            println!("Warning: No jnz_opcode_taken states generated. Test will be skipped.");
            return;
        }

        let jnz_gen = jnz_opcode_taken::ClaimGenerator::new(input_state.casm_states_by_opcode.jnz_opcode_taken);
        ...
        // Base trace.
        let mut mock_tree_builder = mock_commitment_scheme.tree_builder();
        let (jnz_claim, jnz_interaction_gen) = jnz_gen.write_trace(
        ...
        println!("jnz_opcode_taken_claim log_size: {:?}", jnz_claim.log_size);

        // Interaction trace.
        let mut mock_tree_builder = mock_commitment_scheme.tree_builder();
        let jnz_interaction_claim = jnz_interaction_gen.write_interaction_trace(
                    &mut mock_tree_builder,
        ...
        println!("jnz_opcode_taken_interaction_claim.claimed_sum: {:?}", jnz_interaction_claim.claimed_sum);

        let tree_span_provider = &mut TraceLocationAllocator::default();
        let jnz_component = Component::new(
            tree_span_provider,
            Eval {
                eval_id: fnv1a_eval_id_gen("jnz_opcode_taken"),
                claim: jnz_claim.clone(),
                verify_instruction_lookup_elements: relations::VerifyInstruction::dummy(),
                memory_address_to_id_lookup_elements: relations::MemoryAddressToId::dummy(),
                memory_id_to_big_lookup_elements: relations::MemoryIdToBig::dummy(),
                opcodes_lookup_elements: relations::Opcodes::dummy(),
            },
            jnz_interaction_claim.claimed_sum,
        );

        assert_component(&jnz_component, &trace)
    }

    #[test]
    fn test_jnz_opcode_taken_trace_gen_by_cpu_and_verify_by_cuda() {
        let instructions = casm! {
            [ap] = 1, ap++;
            jmp rel 4 if [ap-1] != 0;
            [ap] = 0, ap++;
        ...
        let jnz_gen = jnz_opcode_taken::ClaimGenerator::new(
                input_state.casm_states_by_opcode.jnz_opcode_taken,
            );
        ...
        println!("jnz_opcode_taken_claims log_size: {:?}", jnz_claim.log_size);
        ...
        let eval_ptr = &jnz_component.eval as *const _ as *mut std::os::raw::c_void;
        unsafe {
            stwo::stwo_cuda::bindings::evaluate_constraint_quotients_on_domain(
                ...
                jnz_claim.log_size as u32,
                jnz_claim.log_size as u32,
                jnz_component.info.n_constraints as u32,
                jnz_component.info.logup_counts.iter().map(|(_, &count)| count).sum::<usize>() as u32,
                eval_ptr,
                CudaSecureField::from(
                    jnz_interaction_claim.claimed_sum
                        / BaseField::from_u32_unchecked(1 << jnz_claim.log_size)
                ),
    ...
    #[test]
    fn test_jnz_opcode_taken_trace_gen_by_cuda_and_verify_by_cpu() {
        let instructions = casm! {
            [ap] = 1, ap++;
            jmp rel 4 if [ap-1] != 0;
            [ap] = 0, ap++;
        ...
        let jnz_gen = jnz_opcode_taken_cuda::CudaClaimGenerator::new(input_state.casm_states_by_opcode.jnz_opcode_taken);
    ...
    #[test]
    fn test_jnz_opcode_taken_trace_gen_by_cuda_and_verify_by_cuda() {
        let instructions = casm! {
            [ap] = 1, ap++;
            jmp rel 4 if [ap-1] != 0;
            [ap] = 0, ap++;
