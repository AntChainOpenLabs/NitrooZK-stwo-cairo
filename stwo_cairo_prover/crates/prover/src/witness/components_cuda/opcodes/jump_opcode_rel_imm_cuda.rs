#![allow(unused_parens)]
use cairo_air::components::jump_opcode_rel_imm::{Claim, InteractionClaim};
...
impl CudaClaimGenerator {
    pub fn new(inputs: Vec<InputType>) -> Self {
        ...
        // Pad with FIRST row to match SIMD behavior
...
fn write_trace_cuda(
    ...
    let memory_id_to_big_transposed_big_values_vec = memory_id_to_big_state.transposed_big_values
        ...
        .collect_vec();
    unsafe {
        bindings_airs::generate_jump_opcode_rel_imm_traces(
            traces_vec.as_ptr(),
...
impl CudaInteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        unsafe {
            ...
            bindings_airs::generate_jump_opcode_rel_imm_interaction_traces(
...
pub mod tests {
    ...
    use crate::witness::components::{memory_address_to_id, memory_id_to_big, verify_instruction};
    use crate::witness::components_cuda::jump_opcode_rel_imm_cuda;
    ...
    use cairo_air::components::jump_opcode_rel_imm::Eval;
    use stwo::core::fields::m31::M31;
    use stwo_cairo_common::preprocessed_columns::preprocessed_trace::testing_preprocessed_tree;
    use crate::witness::components::jump_opcode_rel_imm;
    use cairo_lang_casm::casm;
    use crate::test_utils::input_from_plain_casm;
    use cairo_air::components::jump_opcode_rel_imm::Component;
    use itertools::Itertools;
    use stwo::stwo_cuda::base_field_vec::BaseFieldVec;
    ...
    #[test]
    fn test_jump_opcode_rel_imm_cpu_ref() {
        ...
        // Check that we have jump_opcode_rel_imm states
        if input_state.casm_states_by_opcode.jump_opcode_rel_imm.is_empty() {
            println!("Warning: No jump_opcode_rel_imm states generated. Test will be skipped.");
            return;
        }

        println!("Generated {} jump_opcode_rel_imm states", input_state.casm_states_by_opcode.jump_opcode_rel_imm.len());
    }

    #[test]
    fn test_jump_opcode_rel_imm_trace_gen_by_cpu_and_verify_by_cuda() {
        let instructions = casm! {
            jmp rel 2;
            [ap] = [ap-1] + 3, ap++;
        }
        ...
        println!("CPU ref test: Generated {} jump_opcode_rel_imm states", input_state.casm_states_by_opcode.jump_opcode_rel_imm.len());
    }

    #[test]
    fn test_jump_opcode_rel_imm_trace_gen_by_cuda_and_verify_by_cpu() {
        let instructions = casm! {
            jmp rel 2;
            [ap] = [ap-1] + 3, ap++;
        }
        ...
        let jump_gen = jump_opcode_rel_imm_cuda::CudaClaimGenerator::new(input_state.casm_states_by_opcode.jump_opcode_rel_imm);
        ...
        println!("jump_opcode_rel_imm_claim log_size: {:?}", jump_claim.log_size);
        ...
        println!("jump_opcode_rel_imm_interaction_claim.claimed_sum: {:?}", jump_interaction_claim.claimed_sum);
    ...
    #[test]
    fn test_jump_opcode_rel_imm_trace_gen_by_cuda_and_verify_by_cuda() {
        let instructions = casm! {
            jmp rel 2;
            [ap] = [ap-1] + 3, ap++;
        }
