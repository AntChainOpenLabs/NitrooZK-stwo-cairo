#![allow(unused_parens)]
use cairo_air::components::jump_opcode_rel::{Claim, InteractionClaim};
...
fn write_trace_cuda(
    ...
    let memory_id_to_big_transposed_big_values_vec = memory_id_to_big_state.transposed_big_values
        ...
        .collect_vec();
    unsafe {
        bindings_airs::generate_jump_opcode_rel_traces(
            traces_vec.as_ptr(),
...
impl CudaInteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        unsafe {
            ...
            bindings_airs::generate_jump_opcode_rel_interaction_traces(
...
pub mod tests {
    ...
    use crate::witness::components::{memory_address_to_id, memory_id_to_big, verify_instruction};
    use crate::witness::components_cuda::jump_opcode_rel_cuda;
    ...
    use cairo_air::components::jump_opcode_rel::Eval;
    use stwo::core::fields::m31::M31;
    use stwo_cairo_common::preprocessed_columns::preprocessed_trace::testing_preprocessed_tree;
    use crate::witness::components::jump_opcode_rel;
    use cairo_lang_casm::casm;
    use crate::test_utils::input_from_plain_casm;
    use cairo_air::components::jump_opcode_rel::Component;
    use itertools::Itertools;
    use stwo::stwo_cuda::base_field_vec::BaseFieldVec;
    ...
    // Note: jump_opcode_rel (jmp rel [memory]) is rarely used in standard Cairo programs.
    // Most Cairo programs use immediate relative jumps (jmp rel <imm>) instead.
    // The CASM macro doesn't support the unconditional `jmp rel [memory]` syntax,
    // so these tests are marked as ignored. The CUDA implementation is complete.
    #[test]
    #[ignore = "Relative jump with memory deref (jmp rel [mem]) not supported in CASM macro"]
    fn test_jump_opcode_rel_cpu_ref() {
        println!("jump_opcode_rel tests are skipped:");
        println!("- The CASM macro doesn't support `jmp rel [memory]` syntax");
        println!("- The CUDA implementation is complete but difficult to test with simple CASM");
    }

    #[test]
    #[ignore = "Relative jump with memory deref (jmp rel [mem]) not supported in CASM macro"]
    fn test_jump_opcode_rel_trace_gen_by_cpu_and_verify_by_cuda() {
        println!("jump_opcode_rel tests are skipped");
    }

    #[test]
    #[ignore = "Relative jump with memory deref (jmp rel [mem]) not supported in CASM macro"]
    fn test_jump_opcode_rel_trace_gen_by_cuda_and_verify_by_cpu() {
        println!("jump_opcode_rel tests are skipped");
    }

    #[test]
    #[ignore = "Relative jump with memory deref (jmp rel [mem]) not supported in CASM macro"]
    fn test_jump_opcode_rel_trace_gen_by_cuda_and_verify_by_cuda() {
        println!("jump_opcode_rel tests are skipped");
    }
}
