#![allow(unused_parens)]
use cairo_air::components::assert_eq_opcode::{Claim, InteractionClaim};

use crate::witness::prelude::*;

use super::super::{memory_address_to_id_cuda, memory_id_to_big_cuda, verify_instruction_cuda};
use crate::witness::components::{memory_address_to_id, verify_instruction};
...
impl CudaClaimGenerator {
    ...
    pub fn write_trace(
        ...
        // memory_address_to_id has 2 lookups, each with 1 field
...
fn write_trace_cuda(
    ...
    let memory_id_to_big_transposed_big_values_vec = memory_id_to_big_state.transposed_big_values
        ...
        .collect_vec();
    unsafe {
        bindings_airs::generate_assert_eq_opcode_traces(
            traces_vec.as_ptr(),
...
impl CudaInteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        unsafe {
            ...
            bindings_airs::generate_assert_eq_opcode_interaction_traces(
...
pub mod tests {
    ...
    use crate::witness::components_cuda::assert_eq_opcode_cuda;
    ...
    use cairo_air::components::assert_eq_opcode::Eval;
    ...
    use crate::witness::components::assert_eq_opcode;
    use crate::witness::components::verify_instruction;
    use cairo_lang_casm::casm;
    use crate::test_utils::input_from_plain_casm;
    use cairo_air::components::assert_eq_opcode::Component;
    use itertools::Itertools;
    use stwo::stwo_cuda::base_field_vec::BaseFieldVec;
    ...
    #[test]
    fn test_assert_eq_opcode_cpu_ref() {
        ...
        assert!(!input_state.casm_states_by_opcode.assert_eq_opcode.is_empty());

        let assert_eq = assert_eq_opcode::ClaimGenerator::new(input_state.casm_states_by_opcode.assert_eq_opcode);
        ...
        // Base trace.
        let mut mock_tree_builder = mock_commitment_scheme.tree_builder();
        let (assert_eq_claim, assert_eq_interaction_gen) = assert_eq.write_trace(
        ...
        println!("assert_eq_opcode_claim log_size: {:?}", assert_eq_claim.log_size);

        // Interaction trace.
        let mut mock_tree_builder = mock_commitment_scheme.tree_builder();
        let assert_eq_interaction_claim = assert_eq_interaction_gen.write_interaction_trace(
                    &mut mock_tree_builder,
        ...
        println!("assert_eq_opcode_interaction_claim.claimed_sum: {:?}", assert_eq_interaction_claim.claimed_sum);
    ...
    #[test]
    fn test_assert_eq_opcode_trace_gen_by_cpu_and_verify_by_cuda() {
        let instructions = casm! {
            [ap] =  8, ap++;
            [ap] =  8, ap++;
            [ap+2] = [fp + 1];
        ...
        let assert_eq= assert_eq_opcode::ClaimGenerator::new(
                input_state.casm_states_by_opcode.assert_eq_opcode,
            );
        ...
        println!("assert_eq_opcode_claims log_size: {:?}", assert_eq_claim.log_size);
        ...
        unsafe {
            stwo::stwo_cuda::bindings::evaluate_constraint_quotients_on_domain(
                ...
                assert_eq_claim.log_size as u32,
                assert_eq_claim.log_size as u32,
                ...
                CudaSecureField::from(
                    assert_eq_interaction_claim.claimed_sum
                        / BaseField::from_u32_unchecked(1 << assert_eq_claim.log_size)
                ),
    ...
    #[test]
    fn test_assert_eq_opcode_trace_gen_by_cuda_and_verify_by_cpu() {
        let instructions = casm! {
            [ap] =  8, ap++;
            [ap] =  8, ap++;
            [ap+2] = [fp + 1];
        ...
        let assert_eq = assert_eq_opcode_cuda::CudaClaimGenerator::new(input_state.casm_states_by_opcode.assert_eq_opcode);
    ...
    #[test]
    fn test_assert_eq_opcode_trace_gen_by_cuda_and_verify_by_cuda() {
        let instructions = casm! {
            [ap] =  8, ap++;
            [ap] =  8, ap++;
            [ap+2] = [fp + 1];
