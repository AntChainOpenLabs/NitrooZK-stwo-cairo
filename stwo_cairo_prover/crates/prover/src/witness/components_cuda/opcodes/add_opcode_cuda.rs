#![allow(unused_parens)]
use cairo_air::components::add_opcode::{Claim, InteractionClaim};
...
impl CudaClaimGenerator {
    ...
    pub fn write_trace(
        ...
        // Note: CUDA kernel uses n_rows for both enabler AND sub_component_inputs bounds.
        // Passing padded_size would break the enabler. We keep n_rows here and manually
        // add multiplicities for padded rows' memory lookups in cairo_cuda.rs.
        let (trace, lookup_data, sub_component_inputs) = write_trace_cuda(
            self.n_rows,  // Use actual n_rows (CUDA kernel limitation)
            packed_inputs,
        ...
        (
            ...
            CudaInteractionClaimGenerator {
                n_rows : self.n_rows,  // Keep actual n_rows for enabler in interaction trace
                log_size,
                lookup_data,
            },
        )
...
struct CudaSubComponentInputs {
    ...
    memory_id_to_big: [memory_address_to_id_cuda::CudaPackedInputType; 3],
}
...
fn write_trace_cuda(
    ...
    let memory_id_to_big_transposed_big_values_vec = memory_id_to_big_state.transposed_big_values
        ...
        .collect_vec();
    unsafe {
        bindings_airs::generate_add_opcode_traces(
            traces_vec.as_ptr(),
...
impl CudaInteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        unsafe {
            ...
            bindings_airs::generate_add_opcode_interaction_traces(
...
pub mod tests {
    ...
    use crate::witness::components_cuda::add_opcode_cuda;
    ...
    use cairo_air::components::add_opcode::Eval;
    ...
    use crate::witness::components::add_opcode;
    use crate::witness::components::verify_instruction;
    use cairo_air::components::add_opcode::Component;
    ...
    fn test_add_opcode_cpu_ref() {
        ...
        // Use add_opcode_small since small adds are optimized differently
        assert!(!input_state.casm_states_by_opcode.add_opcode.is_empty());
        let add = add_opcode::ClaimGenerator::new(input_state.casm_states_by_opcode.add_opcode);
        ...
        // Base trace.
        let mut mock_tree_builder = mock_commitment_scheme.tree_builder();
        let (add_claim, add_interaction_gen) = add.write_trace(
        ...
        println!("add_claim log_size: {:?}", add_claim.log_size);

        // Interaction trace.
        let mut mock_tree_builder = mock_commitment_scheme.tree_builder();
        let add_interaction_claim = add_interaction_gen.write_interaction_trace(
                    &mut mock_tree_builder,
        ...
        println!("add_interaction_claim.claimed_sum: {:?}", add_interaction_claim.claimed_sum);
        ...
        let add_components = Component::new(
            ...
            Eval {
                ...
                claim: add_claim.clone(),
                memory_address_to_id_lookup_elements: relations::MemoryAddressToId::dummy(),
                memory_id_to_big_lookup_elements: relations::MemoryIdToBig::dummy(),
                opcodes_lookup_elements: relations::Opcodes::dummy(),
                verify_instruction_lookup_elements: relations::VerifyInstruction::dummy(),
            },
            add_interaction_claim.claimed_sum,
        );
        assert_component(&add_components, &trace)
    }

    #[test]
    fn test_add_opcode_trace_gen_by_cpu_and_verify_by_cuda() {
        let instructions = casm! {
            call rel 2;
            [ap] = 536870909, ap++;
            [ap] = 3, ap++;
        ...
        let add = add_opcode::ClaimGenerator::new(
                input_state.casm_states_by_opcode.add_opcode,
            );
        ...
        println!("add_claims log_size: {:?}", add_claim.log_size);
        ...
        unsafe {
            stwo::stwo_cuda::bindings::evaluate_constraint_quotients_on_domain(
                ...
                CudaSecureField::from(
                    add_interaction_claim.claimed_sum
                        / BaseField::from_u32_unchecked(1 << add_claim.log_size)
                ),
                false,
                true,
            );
    ...
    #[test]
    fn test_add_opcode_trace_gen_by_cuda_and_verify_by_cpu() {
        let instructions = casm! {
            call rel 2;
            [ap] = 536870909, ap++;
            [ap] = 3, ap++;
        ...
        let add = add_opcode_cuda::CudaClaimGenerator::new(input_state.casm_states_by_opcode.add_opcode);
        ...
        // Convert inst_cache Vec<(u32, u128)> to HashMap<M31, u128>
    ...
    #[test]
    fn test_add_opcode_trace_gen_by_cuda_and_verify_by_cuda() {
        let instructions = casm! {
            call rel 2;
            [ap] = 536870909, ap++;
            [ap] = 3, ap++;
