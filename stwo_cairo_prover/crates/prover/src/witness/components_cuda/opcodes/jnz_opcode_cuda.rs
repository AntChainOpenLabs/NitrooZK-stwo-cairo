#![allow(unused_parens)]
use cairo_air::components::jnz_opcode_non_taken::{Claim, InteractionClaim};
...
/// The CUDA kernel generates 37 columns for the unified legacy jnz_opcode.
const CUDA_N_TRACE_COLUMNS: usize = 37;
/// The AIR for jnz_opcode_non_taken expects only 9 columns.
...
impl CudaClaimGenerator {
    ...
    pub fn write_trace(
        ...
        // Extract the 9 AIR columns from 37 CUDA columns: [0..7] + [36] (enabler).
        const AIR_COL_INDICES: [usize; 9] = [0, 1, 2, 3, 4, 5, 6, 7, 36];
        let mut all_evals: Vec<Option<_>> =
            trace.to_evals().to_vec().into_iter().map(Some).collect();
        let selected_evals: Vec<_> = AIR_COL_INDICES
            .iter()
