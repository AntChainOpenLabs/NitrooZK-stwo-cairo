#![allow(unused_parens)]
use cairo_air::components::jnz_opcode::{Claim, InteractionClaim};
...
fn write_trace_cuda(
    ...
    let memory_id_to_big_transposed_big_values_vec = memory_id_to_big_state.transposed_big_values
        ...
        .collect_vec();
    unsafe {
        bindings_airs::generate_jnz_opcode_traces(
            traces_vec.as_ptr(),
...
impl CudaInteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        unsafe {
            ...
            bindings_airs::generate_jnz_opcode_interaction_traces(
...
pub mod tests {
    ...
    use crate::witness::components::{memory_address_to_id, memory_id_to_big, verify_instruction};
    use crate::witness::components_cuda::jnz_opcode_cuda;
    ...
    fn test_jnz_opcode_cpu_ref() {
        ...
        // Check that we have jnz_opcode states
        if input_state.casm_states_by_opcode.jnz_opcode.is_empty() {
            println!("Warning: No jnz_opcode states generated. Test will be skipped.");
            return;
        }

        let jnz_gen = jnz_opcode::ClaimGenerator::new(input_state.casm_states_by_opcode.jnz_opcode);
        ...
        println!("jnz_opcode_claim log_size: {:?}", jnz_claim.log_size);
        ...
        println!("jnz_opcode_interaction_claim.claimed_sum: {:?}", jnz_interaction_claim.claimed_sum);
    ...
    #[test]
    fn test_jnz_opcode_trace_gen_by_cpu_and_verify_by_cuda() {
        let instructions = casm! {
            [ap] = 0, ap++;
            jmp rel 2 if [ap-1] != 0;
            [ap] = 1, ap++;
        ...
        let jnz_gen = jnz_opcode::ClaimGenerator::new(
                input_state.casm_states_by_opcode.jnz_opcode,
            );
        ...
        println!("jnz_opcode_claims log_size: {:?}", jnz_claim.log_size);
    ...
    #[test]
    fn test_jnz_opcode_trace_gen_by_cuda_and_verify_by_cpu() {
        let instructions = casm! {
            [ap] = 0, ap++;
            jmp rel 2 if [ap-1] != 0;
            [ap] = 1, ap++;
        ...
        let jnz_gen = jnz_opcode_cuda::CudaClaimGenerator::new(input_state.casm_states_by_opcode.jnz_opcode);
    ...
    #[test]
    fn test_jnz_opcode_trace_gen_by_cuda_and_verify_by_cuda() {
        let instructions = casm! {
            [ap] = 0, ap++;
            jmp rel 2 if [ap-1] != 0;
            [ap] = 1, ap++;
