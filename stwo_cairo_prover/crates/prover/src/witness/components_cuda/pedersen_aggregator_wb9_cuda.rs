...
///
/// Supports two modes:
/// - `from_gpu_ids`: Direct GPU path — takes 3 ID arrays already on GPU.
/// - Legacy DashMap path (via `new` + `inner`).
pub struct CudaClaimGenerator {
    inner: Option<pedersen_aggregator_window_bits_9::ClaimGenerator>,
...
impl CudaClaimGenerator {
    pub fn new() -> Self {
        Self {
            inner: Some(pedersen_aggregator_window_bits_9::ClaimGenerator::new()),
            gpu_ids: None,
        }
    }

    /// Create from GPU ID arrays produced by pedersen_builtin_narrow.
    ...
    pub fn write_trace(
        ...
        let pem_total_rows = 56 * (1usize << log_size);
