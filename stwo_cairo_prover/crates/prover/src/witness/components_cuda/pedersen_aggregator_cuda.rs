//! Native CUDA trace generation for pedersen_aggregator_window_bits_18 (206 columns).
//!
//! Replaces the previous SIMD-wrapped approach (SimdToCudaAdapter) with a native
//! CUDA kernel that runs on the GPU-resident pedersen table. This eliminates the
//! need for the CPU PEDERSEN_TABLE_18 (~7s init) in the CUDA proving path.
//!
//! Architecture:
...
//! - CUDA kernel generates 206 trace cols + lookup data + sub-component inputs
//! - Sub-component feeds: 3× memory_id_to_big, 4× range_check_8, 28× partial_ec_mul
//! - CudaInteractionClaimGenerator: computes 6 logup columns entirely on GPU via native CUDA
//!   kernel, using lookup data arrays that already reside on the device
...
use crate::witness::components_cuda::{
    memory_id_to_big_cuda, partial_ec_mul_cuda, pedersen_points_table_cuda,
};
...
impl CudaClaimGenerator {
    ...
    pub fn write_trace(
        ...
        // partial_ec_mul: 72 columns × (28 * size) rows, feed via set_cuda_inputs
        // The kernel wrote sub_pem as 72 arrays, each of size 28*trace_size.
        // We need to convert these to PEM's expected format.
        ...
        let pem_inputs: [BaseFieldVec; 72] = sub_component_inputs
            .pem
            .try_into()
            .expect("sub_pem should have 72 columns");
        pem_cuda.set_cuda_inputs(pem_inputs, pem_total_rows);
...
struct CudaLookupData {
    ...
    pem_0: [BaseFieldVec; 73],
    pem_1: [BaseFieldVec; 73],
    pem_2: [BaseFieldVec; 73],
    pem_3: [BaseFieldVec; 73],
...
struct CudaSubComponentInputs {
    ...
    pem: Vec<BaseFieldVec>, // 72 arrays (each 28*trace_size)
}
...
fn write_trace_cuda(
    ...
    let (trace, lookup_data, sub_component_inputs) = unsafe {
        (
            ...
            CudaLookupData {
                ...
                // Store the mults GPU array for the interaction trace (col 5).
                mults,
            },
            CudaSubComponentInputs {
                mem: init_lookup_vec(3, log_size),
                rc8: init_lookup_vec(4, log_size),
                // PEM: 72 columns, each with 28 * trace_size elements
                pem: (0..72)
                    .map(|_| BaseFieldVec::uninitialized(28 * trace_size))
                    .collect(),
            },
        )
    };
    ...
    unsafe {
        bindings_airs::gen_pedersen_aggregator_wb18_trace(
            traces_vec.as_ptr(),
            lk_mem_0_vec.as_ptr(),
            lk_mem_1_vec.as_ptr(),
            lk_mem_2_vec.as_ptr(),
...
/// Generates the 6 logup interaction trace columns.
///
/// Uses a native CUDA kernel to compute all logup columns entirely on GPU,
/// eliminating the GPU→CPU download + SIMD LogupTraceGenerator + re-upload
/// round-trips.
...
impl CudaInteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        unsafe {
            bindings_airs::gen_pedersen_aggregator_wb18_interaction_trace(
