//! CUDA version of partial_ec_mul_window_bits_9 component.
//!
//! Adapted from partial_ec_mul_cuda.rs (window_bits_18 variant).
//! Uses g_pedersen_table_small_columns on GPU (56 cols x 32K rows, ~7MB).
//!
//! Sub-component feeds:
//! - pedersen_points_table_wb9: 1 feed (table index)
...
use crate::witness::components::partial_ec_mul_window_bits_9;
use crate::witness::components_cuda::pedersen_points_table_wb9_cuda;
...
/// Number of input columns (from PackedInputType: 1+1+28+28+28 = 86).
pub const N_INPUT_COLUMNS: usize = 86;

/// Re-export the packed input type from the SIMD component.
pub type PackedInputType = partial_ec_mul_window_bits_9::PackedInputType;
...
pub struct CudaClaimGenerator {
    /// Column-major GPU input columns (86 BaseFieldVec columns).
    /// Populated by set_cuda_inputs() from GPU data (from aggregator kernel).
...
impl CudaClaimGenerator {
    ...
    pub fn write_trace(
        ...
        pedersen_points_table_state: &pedersen_points_table_wb9_cuda::CudaClaimGenerator,
        ...
        assert!(
            self.size > 0,
            "partial_ec_mul_wb9_cuda: write_trace called with 0 inputs"
        );
        ...
        // PPT wb9: 1 feed of [BaseFieldVec; 1]
        let ppt_input: &[pedersen_points_table_wb9_cuda::CudaPackedInputType] = unsafe {
            std::slice::from_raw_parts(
                sub_component_inputs.ppt.as_ptr()
                    as *const pedersen_points_table_wb9_cuda::CudaPackedInputType,
                1,
            )
        };
        pedersen_points_table_state.add_cuda_inputs(ppt_input);

        // rc_9_9: 8 relation variants
        for rel in 0..8 {
...
struct CudaLookupData {
    // Self-interaction lookups (provide/consume) — 87 values each (was 73 for w18)
    partial_ec_mul_0: [BaseFieldVec; 87],
    partial_ec_mul_1: [BaseFieldVec; 87],
    // Pedersen points table lookup
    ppt_0: [BaseFieldVec; 58],
    // rc_20 lookups: 8 variants
    rc_20: [Vec<BaseFieldVec>; 8],
    // rc_9_9 lookups: 8 variants
...
struct CudaSubComponentInputs {
    // PPT wb9: 1 feed of 1 column (table index)
    ppt: [BaseFieldVec; 1],
    // rc_9_9: 8 variant groups
    rc_9_9: [Vec<BaseFieldVec>; 8],
    // rc_20: 8 variant groups
    rc_20: [Vec<BaseFieldVec>; 8],
}
...
fn write_trace_cuda(
    ...
    unsafe {
        bindings_airs::gen_partial_ec_mul_wb9_trace(
            traces_vec.as_ptr(),
            // Lookup data
...
impl CudaInteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        unsafe {
            bindings_airs::gen_partial_ec_mul_wb9_interaction_trace(
